    ...
    def Mutate(self):
        ...
        if sizeOrJoint == 0:
            ...
            if (self.numLinksJoint == 1 or (self.numLinksJoint == 2 and incOrDecSize == 0)):    
                ...
                weights = numpy.zeros((self.numSensorNeurons, self.numMotorNeurons))
                weights[:self.numSensorNeurons -1, :self.numMotorNeurons -1] = self.weights

                # Append new values
                new_weight_row = numpy.random.random((1, self.numMotorNeurons)) * 2 - 1
                weights[-1, :] = new_weight_row

                new_weight_column = numpy.random.random((self.numSensorNeurons, 1)) * 2 - 1
                weights[:, -1] = new_weight_column.flatten()
